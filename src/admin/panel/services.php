...
if ($_SERVER["REQUEST_METHOD"] === "POST" && panel_post_string("action") === "delete") {
  ...
  $save = panel_save_json_file($path, ["items" => $newItems], panel_post_string("sha"), "chore(cms): delete service");
  panel_flash_set($save["ok"] ? "ok" : "err", $save["ok"] ? "Изтрито." : ($save["error"] ?? "Грешка."));
  panel_redirect_with("./services.php");
}
...
?>
    <div class="pk-wrap">
      ...
      <div class="pk-toolbar">
        <div></div>
        <a class="pk-btn pk-btn--sm" href="./service-edit.php">+ Нова услуга</a>
      </div>
        ...
        <?php foreach ($items as $svc): ?>
          <div class="pk-list-item" style="cursor:default;">
            <div class="pk-list-item__body">
              <div class="pk-list-item__title"><?php echo html((string)($svc["title"] ?? "")); ?></div>
              <div class="pk-list-item__meta"><?php echo html((string)($svc["summary"] ?? "")); ?></div>
            </div>
            <div class="pk-actions-inline">
              <a class="pk-btn pk-btn--ghost pk-btn--sm" href="./service-edit.php?slug=<?php echo urlencode((string)($svc["slug"] ?? "")); ?>">Редакция</a>
              <form method="post" onsubmit="return confirm('Изтриване?');">
                <input type="hidden" name="csrf" value="<?php echo html(csrf_token()); ?>" />
                <input type="hidden" name="action" value="delete" />
                <input type="hidden" name="slug" value="<?php echo html((string)($svc["slug"] ?? "")); ?>" />
